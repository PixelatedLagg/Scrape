            ...
            if (args.Length == 2)
            {
                switch (args[0])
                {
                    ...
                    case "run":
                        ...
                        Directory.CreateDirectory($"{Path.Combine(Directory.GetCurrentDirectory(), @"..\..")}\\{Global.OutputFolderName}");
                        foreach (string file in Directory.GetFiles(Path.Combine(Directory.GetCurrentDirectory(), @"..\..")))
                        {
                            if (Path.GetExtension(file) == ".srp")
                            {
                                ...
                                compiler.Compile();
                                using (StreamWriter writer = new StreamWriter($"{Path.Combine(Directory.GetCurrentDirectory(), @"..\..")}\\{Global.OutputFolderName}\\{Path.GetFileNameWithoutExtension(file)}.cpp")) 
                                {
